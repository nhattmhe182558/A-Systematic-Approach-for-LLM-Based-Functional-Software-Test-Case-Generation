...
\textbf{Cross-method comparison, same input (4LV/RetailOnboardPro
sample).} Figures F1--F2 plot IntelliTest (flash, pro) directly against
all three baselines on the identical 4LV sample, using each method's own
...
Baseline 1 (Augusto) & \textbf{569,337} & \textbf{\$1.090} & 27 &
\textbf{21,087} \\
...
\tightlist
\item
  \textbf{Baseline 1 (Augusto) is the most expensive method on this
  input} --- 569,337 tokens / \$1.09, more than any IntelliTest
  configuration, driven by its highest average tokens-per-call (21,087):
  its Few-Shot+CoT prompts embed the full example test-case bank on
  every RQ2 call (leave-one-out cross-validation resends a large example
  set each time), which is expensive but is also \emph{by design}
  (verbatim to the original method).
\item
  \textbf{Baseline 3 (Bhatia) is by far the cheapest} (10,858 tokens /
  \$0.019, only 3 calls) because its conversational design amortizes the
  SRS across a single chat session and asks for all test cases per use
  case in one shot --- but the fidelity audit (§IV-A baselines) shows
  this comes at the cost of shallower, less-structured output (a flat
  6-column Markdown table, no BVA/Pairwise optimization, no navigational
  grounding) compared with IntelliTest's ISO-style, stateful output.
\item
  \textbf{IntelliTest's own model choice matters more than the choice of
  baseline vs.~IntelliTest}: pro (66,774 tokens) is cheaper on this
  small input than every baseline except Bhatia, while flash (429,097
  tokens) sits between Milchevski and Augusto. This says less about
  ``IntelliTest vs.~baselines'' in the abstract and more about ``cost
  scales with how much structure and cross-validation a method builds
  into every call'' --- the deterministic algorithms IntelliTest layers
  on top of the LLM (EP, BVA, Pairwise) are cheap; what varies is how
  verbose each method's \emph{prompting strategy} is per call.
\end{enumerate}
...
\textbf{Head-to-head, same SUT: agentic TEAA vs.~static-script baseline
(Figures F5--F6).} Directly comparing the two execution paradigms on the
same live FullTeaching SUT:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{TEAA agent} (agentic, self-healing) & 166 & 70 &
\textbf{42.2\%} \\
\textbf{Static script} (Cypress/AutoUAT-style) & 169 & 26 &
\textbf{15.4\%} \\
\end{longtable}
}

The agentic approach passes \textbf{\textasciitilde2.7× the proportion}
of test cases that the static-script baseline does on a comparably-sized
suite. The \emph{reason} differs sharply between paradigms (Figure F6):
the static script's failures are overwhelmingly (78\%, 132/169) plain
\textbf{Failed} outcomes attributable to brittle selectors and
timing/race conditions against a dynamic Angular UI, with no mechanism
to recover once a selector goes stale. The TEAA agent's failures split
into \textbf{Web/DOM failures (37\%, 61/96)} --- the same class of
brittleness, but partially mitigated by DOM re-abstraction on each step
--- and \textbf{LLM failures (21\%, 35/96)}, a \emph{new} failure mode
introduced by the agent's own reasoning (e.g., choosing a plausible but
non-existent element \texttt{id}). In other words, the agentic approach
trades a fraction of the static approach's brittleness failures for a
smaller number of reasoning failures, and nets a substantially higher
pass rate. This is the central Phase-2 argument for TEAA over a
static-script alternative on a dynamic SUT, though it does not eliminate
the brittleness problem --- it converts most of it into a partially
self-healing form.

\textbf{Where the agent still struggles (Figure F7).} Breaking down
TEAA's own success rate by action type shows the same brittleness
surfacing internally: \texttt{done} (task self-termination) succeeds
68.8\% of the time, but \texttt{click} succeeds only 33.0\% and
\texttt{input} only 25.7\% --- the two actions that require the agent to
have correctly identified a \emph{specific} element via a selector it
inferred from the abstracted DOM. \texttt{upload} (n=8, 50\%) is too
small a sample to generalize. This localizes the agent's weakness
precisely: not ``does the agent decide correctly'' (its \texttt{done}
signal is comparatively reliable) but ``does the agent's selector match
the live DOM,'' which is exactly the self-healing/selector-repair
problem the newest literature (arXiv:2603.20358, arXiv:2605.01471) is
now targeting.
...
\textbf{Phase 1 --- Generation (IntelliTest vs.~Baselines 1/2/3):} -
\textbf{F1} Total tokens and USD cost per method, same
4LV/RetailOnboardPro input (IntelliTest flash/pro vs. Baseline 1/2/3)
--- the primary cross-method cost comparison. - \textbf{F2} Number of
LLM calls and average tokens/call per method --- efficiency view
explaining \emph{why} costs differ (Baseline 1's cross-validation
prompts are the most token-hungry per call). - \textbf{F3} IntelliTest
model comparison: flash vs.~pro on the full-scale FullTeaching run
(tokens, cost, calls). - \textbf{F4} IntelliTest tokens by pipeline
stage (FullTeaching, flash) --- \texttt{test\_case\_generator}
dominates.

\textbf{Phase 2 --- Execution (TEAA agent vs.~static-script baseline):}
- \textbf{F5} Headline comparison: TEAA (42.2\% success, N=166)
vs.~static-script/Cypress baseline (15.4\%, N=169) on the same live
FullTeaching SUT. - \textbf{F6} Failure-mode attribution for each
paradigm --- static script: 78\% plain failures (brittle selectors/race
conditions); TEAA: 37\% web/DOM failures + 21\% LLM reasoning failures.
- \textbf{F7} TEAA success rate by browser-action type --- \texttt{done}
69\% vs.~\texttt{click} 33\% / \texttt{input} 26\%, localizing the
agent's weakness to selector grounding, not decision-making. -
\textbf{F8} TEAA pass/fail by business process (BP1 carries 132/166 of
all executed cases).
