...
class TestAgent:
    ...
    def extract_testcase(self,tcs_path = "tc_process"):
    ...
    def walkthrough(self, timeout_seconds = 600):
        ...
        for bussiness_process in bussiness_process_dict.keys():
            ...
            for use_case in bussiness_process_dict[bussiness_process]:
                ...
                # Filter for success path first
                print(f"Trying to extract success path for use case {use_case}")
                for testcase_idx, testcase in enumerate(current_use_case["test_cases"]):
                    
                    if testcase["expected_outcome"].lower() == 'true':
                        print(f"Success extract success path for {use_case}")
                        success_tc = current_use_case["test_cases"].pop(testcase_idx)
                ...
                # Dealing with false testcase
                for testcase in tqdm(current_use_case["test_cases"], total = len(current_use_case["test_cases"])):
                    if testcase["expected_outcome"].lower() == 'false':
                        try:
                            execute_log = self.generate_false_testcase(success_actions=success_reuseable_testcase_object,success_test_case=success_tc, false_test_case=testcase)
                            execute_log = execute_log["execute_log"]
                            reuseable_testcase_object = {"execute_log" : execute_log, "test_case_info" : testcase}
                            reuseable_testcase_list.append(reuseable_testcase_object)
                        except:
                            reuseable_testcase_object = {"execute_log" : [], "test_case_info" : testcase}
                            reuseable_testcase_list.append(reuseable_testcase_object)

                folder_path = f"generated_testcase_action/{bussiness_process}"
                os.makedirs(folder_path, exist_ok=True)
                file_path = f"{folder_path}/{use_case}"

                # Rerun and Evaluate false testcase
                for testcase in tqdm(reuseable_testcase_list, total=len(reuseable_testcase_list)):
                    if testcase["test_case_info"]["expected_outcome"].lower() == "true":
                        continue
                    driver = webdriver.Chrome(options=options)
                    driver.get(url=self.web_url)
                    ...
                    result = self.evaluate_test_execution(test_case_description=testcase["test_case_info"], 
                                                execution_log=testcase["execute_log"],
                                                html_before=html_before,
                                                html_after=html_after,
                                                screenshot_before=screenshot_before,
                                                screenshot_after=screenshot_after,
                                                visible_element=visible_elements)
                    testcase["evaluation"] = result
                    driver.quit()

                with open(file_path, "w", encoding='utf-8') as f:
                    json.dump(reuseable_testcase_list, f, indent=2, ensure_ascii=False)
                with open("success_happy_path.json", "w", encoding='utf-8') as f:
